...
// Read the global system flags (returns object like { enabled: boolean })
async function getSystem() {
  const sql = getSql();
  // Ensure table exists
  await sql`CREATE TABLE IF NOT EXISTS system_flags (key text PRIMARY KEY, value jsonb)`;
  const res = await sql`SELECT value FROM system_flags WHERE key = 'global'`;
  if (!res || res.length === 0) return { enabled: true };
  return res[0].value || { enabled: true };
}
...
// Set system enabled/disabled globally and enqueue per-zone commands for immediate action.
async function setSystemEnabled(enabled) {
  const sql = getSql();

  // Ensure system_flags table exists (simple key/value store)
  await sql`CREATE TABLE IF NOT EXISTS system_flags (key text PRIMARY KEY, value jsonb)`;

  const value = { enabled };
  // Upsert the global flag
  await sql`
    INSERT INTO system_flags (key, value)
    VALUES ('global', ${value}::jsonb)
    ON CONFLICT (key) DO UPDATE SET value = ${value}::jsonb
  `;

  // Enqueue a command for each zone so devices act immediately on next poll
  const rows = await sql`SELECT zone_id FROM zones`;
  if (!rows || rows.length === 0) return [];

  const results = [];
  for (const r of rows) {
    const zoneId = r.zone_id;
    const cmd = { type: enabled ? 'enable' : 'disable', reason: 'global', ts: nowISO() };
    await queueCommand(zoneId, cmd);
    results.push({ zoneId, command: cmd });
  }
